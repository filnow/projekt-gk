...
std::vector<Core::RenderContext> contexts;
...
glm::mat4 lightVP = glm::ortho(-2.f, 2.f, -2.f, 4.f, 2.0f, 15.0f) * glm::lookAt(sunPos, sunPos - sunDir, glm::vec3(0, 1, 0));
...
void drawObjectPBR(Core::RenderContext& context, glm::mat4 modelMatrix, glm::vec3 color, float roughness, float metallic, glm::mat4 viewProjection = lightVP) {
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "lightVP"), 1, GL_FALSE, (float*)&viewProjection);
...
void renderShadowapSun() {
	...
	for (Core::RenderContext model : contexts) {
		drawObjectDepth(model, lightVP, glm::mat4());
	}
...
void renderScene(GLFWwindow* window)
{
	...
	drawObjectPBR(models::tableContext, glm::mat4(), glm::vec3(0.402978f, 0.120509f, 0.057729f), 0.2f, 0.0f);
...
void loadModelToContext(std::string path, Core::RenderContext& context)
{
	...
	const aiScene* scene = import.ReadFile(path.c_str(), aiProcess_CalcTangentSpace |
...
void init(GLFWwindow* window)
{
	...
	loadModelToContext("./models/coffeeTable.obj", models::tableContext);
	loadCubemap(faces);
	initDepthMap();
}
